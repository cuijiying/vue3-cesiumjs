...
export function useWindField(viewerRef: ShallowRef<Cesium.Viewer | null>) {
  const isVisible = ref(true)
  const isLoading = ref(false)
  const particleCount = ref(2000)
  const speedFactor = ref(0.5)
  const fadeOpacity = ref(0.93)
  const lineWidth = ref(1.5)
  const colorScale = ref<WindColorScaleName>('rainbow')
  const windData = shallowRef<WindFieldData | null>(null)
  ...
  function loadData(cols = 64, rows = 64) {
    isLoading.value = true
    windData.value = generateWindField(cols, rows)
    initParticles()
  ...
  function resetParticle(p: WindParticle) {
    const d = windData.value!
  ...
  function getSpeedColor(speed: number, alpha: number): string {
    ...
    const stops = WIND_COLOR_SCALES[colorScale.value]
  ...
  function animate() {
    ...
    for (const p of particles) {
      const wind = interpolateWind(data, p.lon, p.lat)
      if (!wind) {
        resetParticle(p)
        continue
      }
  ...
  function setParticleCount(count: number) {
    particleCount.value = Math.max(100, Math.min(5000, count))
  }

  function setSpeedFactor(val: number) {
    speedFactor.value = Math.max(0.1, Math.min(3, val))
  }
  ...
  function setLineWidth(val: number) {
    lineWidth.value = Math.max(0.5, Math.min(4, val))
  }

  function setColorScale(name: WindColorScaleName) {
